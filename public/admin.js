        ...
        if (obj.type === 'param' && obj.data.valueType === 'pixaroma_editor') {
            ...
            div.innerHTML = `<div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-2">
                    <button id="btn-sync-${key}" class="p-1 hover:bg-slate-700 rounded text-blue-400 transition-all" title="Sync from Server">
            ...
            const syncBtn = div.querySelector(`#btn-sync-${key}`);
            syncBtn.onclick = async () => {
                if (!currentWorkflowId) return;
                syncBtn.classList.add('animate-spin');
                try {
                    const res = await fetch(`/api/workflows/load/${currentWorkflowId}`, { method: 'POST' });
                    const data = await res.json();
                    if (data.success && data.originalValues[key]) {
                        let val = data.originalValues[key];
                        if (typeof val === 'string' && (val.startsWith('{') || val.startsWith('['))) {
                            try { val = JSON.parse(val); } catch(e) {}
                        }
                        parameters[key] = data.originalValues[key];
                        window._pixaroma_node_data.set(String(obj.data.nodeId), val);
                        alert('Synchronized with server state');
                    }
                } catch(e) { console.error(e); }
                finally { syncBtn.classList.remove('animate-spin'); }
            };
